...
void setup() {
  ...
  strip_tunel.begin();
  strip_tunel.show(); // Initialize all pixels to 'off'
...
//    d =random
  ...
  case 3:
    ...
    for( int i = 0; i < strip_tunel.numPixels(); i++ )  {
      int d = map(n, 1, strip_tunel.numPixels(), 200, 20);
      sparkling( n, d, TUNEL_STRIP );
    }
    break;
  case 4:
    t = random( 40, 200 );    
...
int sparkling( int n, int d, int s ) {
  ...
  for (int i=0; i < strip_tunel.numPixels(); i++) {
    if( random( n ) < ( n - m) )
      strip_tunel.setPixelColor( i, 0 );    
    else
      strip_tunel.setPixelColor( i, white );  
  }
  strip_tunel.show();
  delay( d );
}
